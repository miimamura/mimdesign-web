...
| `fujimoto_sp.png` | `work-farm.html`（スマホ実画面） |
| `chocodesign_sp.png` | `work-corporate.html`（スマホ実画面） |
| `takeuhi_sp.png` | 未配置（治療院が非表示のため） |

### まだ足りていないもの

- **PC 版のフルページ** — 農園・コーポレートの冒頭画像は、いまもモックアップからの
  小さな切り出しのままです。スマホと同じ手順（Chrome の Capture full size screenshot）で
  PC 幅のものを撮っていただけると差し替えます
- 各案件サイトの**下層ページ**キャプチャ
- コーポレートサイトのフルモックアップ

### 置き場所について

ファイルは**このフォルダ（`assets/images/gallery/`）に置いてください。**
リポジトリのルート直下でも私が移動するので支障はありませんが、
アップロード画面のリンクはこのフォルダを指しています。
